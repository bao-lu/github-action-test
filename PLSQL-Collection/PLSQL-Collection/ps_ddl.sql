...
/
b.otx_sync_col IS 'Oracle Text Sync Column: A dummy column used as both the base column for multi-column context indexes, as well as the column that must be updated to Y whenever any data is changed or added so that the Oracle Text CTX_DDL.sync procedure will pick up the changes.';
